...
import org.phpmaven.pear.library.IDependency.DependencyType;
            ...
            try {
                ...
                for (final Xpp3Dom child : dom.getChildren()) {
                    if ("a".equals(child.getName())) {
                        this.apiVersion = this.getPearUtility().createVersion();
    ...
    private void processPackageXml(
        ...
        for (final Xpp3Dom child : dom.getChildren()) {
            if ("name".equals(child.getName())) {
                ...
                final IMaintainer maintainer = this.getPearUtility().createMaintainer();
...
//            }
    ...
    private void fetchOldDeps(final List<IDependency> required, final List<IDependency> optional, final Xpp3Dom rchild) throws PhpCoreException {
        for (final Xpp3Dom dchild : rchild.getChildren()) {
            if ("dep".equals(dchild.getName())) {
                ...
                final IDependency dep = this.getPearUtility().createDependency();
                if (isOptional) {
                    optional.add(dep);
                } else {
                    required.add(dep);
	...
	public IPearUtility getPearUtility() {
		return this.getPackage().getPearUtility();
	}
