 ...
 * @since 2.0.1
 */
@Component(role = IApacheController.class, instantiationStrategy = "per-lookup", hint = "V2.2")
@BuildPluginConfiguration(groupId = "org.phpmaven", artifactId = "maven-httpd-control-api")
public class IxController22 implements IApacheController {
    ...
    public boolean isDaemonActive() throws CommandLineException {
        if (this.pidFile.exists()) {
            try {
                ...
                final String ps = ExecutionUtils.executeCommand(null, "ps aux");
