 ...
 * 
 * @author mepeisen
 * @goal print-include-path
 * @requiresDependencyResolution compile
 */
public class PrintInclude extends AbstractMojo {
    ...
    public void execute() throws MojoExecutionException, MojoFailureException {
    	try {
            ...
            if (this.format == null || "toString".equals(this.format)) {
            	result = String.valueOf(execution.getExecutionConfiguration().getIncludePath());
            } else if ("phpstring".equals(this.format)) {
            	final StringBuffer includePath = new StringBuffer();
            	boolean first = true;
            	for (final String path : execution.getExecutionConfiguration().getIncludePath()) {
            		if (first) {
            			includePath.append("'");
            		} else {
