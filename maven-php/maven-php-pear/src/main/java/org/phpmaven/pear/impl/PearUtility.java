...
public class PearUtility extends org.phpmaven.pear.library.impl.PearUtility implements IMavenPearUtility {
	 ...
	 */
	@Override
	public String convertMavenVersionToPearVersion(final String src) {
	 ...
	 */
	@Override
	public String convertPearVersionToMavenVersion(final String src) {
	...
	private Artifact resolveArtifact(final String groupId, final String artifactId, final String version,
		...
		final Set<ArtifactRepository> setRepos = new HashSet<ArtifactRepository>(this.session.getRequest()
		...
		if (!result.isSuccess()) {
			throw new PhpCoreException("dependency resolution failed for " + groupId + ":" + artifactId + ":" + version);
		}
		...
		try {
			...
			for (final org.sonatype.aether.graph.Dependency dep : drres.getDependencies()) {
				...
				if (!deps.containsKey(key)) {
					...
					final org.sonatype.aether.version.Version ver = SCHEME.parseVersion(dep.getArtifact().getVersion());
					final org.sonatype.aether.version.Version ver2 = SCHEME.parseVersion(dep2.getArtifact()
							.getVersion());
			...
			for (final org.sonatype.aether.graph.Dependency dep : deps.values()) {
				...
				if (ignoreCore
						&& this.isMavenCorePackage(dep.getArtifact().getGroupId(), dep.getArtifact().getArtifactId())) {
					// ignore core packages
					continue;
				}
				this.resolveTgz(dep.getArtifact().getGroupId(), dep.getArtifact().getArtifactId(), dep.getArtifact()
						.getVersion(), filesToInstall, ignoreCore);
				final Artifact depPomArtifact = this.resolveArtifact(dep.getArtifact().getGroupId(), dep.getArtifact()
						.getArtifactId(), dep.getArtifact().getVersion(), "pom", null);
	 ...
	 */
	@Override
	public void installFromMavenRepository(final String groupId, final String artifactId, final String version)
			throws PhpException {
	 ...
	 */
	@Override
	public void installCoreFromMavenRepository(final String groupId, final String artifactId, final String version)
			throws PhpException {
	...
	private void resolveChannels(final MavenProject project) throws PhpException {
		...
		if (build != null) {
			for (final Plugin plugin : build.getPlugins()) {
				if ("org.phpmaven".equals(plugin.getGroupId()) && "maven-php-plugin".equals(plugin.getArtifactId())) {
	...
	public boolean isMavenCorePackage(final String groupId, final String artifactId) {
		if (("net.php".equals(groupId))
				&& ("Archive_Tar".equals(artifactId) || "Console_Getopt".equals(artifactId)
						|| "PEAR".equals(artifactId) || "Structures_Graph".equals(artifactId) || "XML_Util"
							.equals(artifactId))) {
			return true;
		}
		return false;
